...
//-------------------------------
 ...
 END_CALCS
  ...
  !! ECHO(EnvYrs)
  !! NenvData = 0;
  !! for (int i = 1; i<=NenvIndics; i++) NenvData += (EnvYrs(i,2)-EnvYrs(i,1)+1);
...
// ================================================================================================
 ...
 END_CALCS
  ...
  !! nslx_cols_in = 16;                                    //> number of columns in the control file to be read in
 ...
 // Load the parameters into their own ragged matrix
  ...
  ivector slx_envlink(1,nslx_pars);                        ///> type of environmental link
  ivector slx_envvar(1,nslx_pars);                         ///> link to environmental parameter
  ivector slx_randwlk(1,nslx_pars);                        ///> is there a random walk   
  ivector slx_timeVar(1,nslx);
     ...
     for ( int j = 1; j <= slx_cols(k); j++ )               ///> read parameters for each pattern
      {
       ...
       slx_randwlk(jj) = slx_control_in(jj,16);
       if (slx_envlink(jj)>0) { nslx_envpars += 1; slx_timeVar(k) = 1; }
     }
   }
...
// Warning - Andre modified the code so initial values for selex is not re-set in the PARAMETER_SECTION
// 2022-08-22; (Version 2.01.M); Read in environmental data
