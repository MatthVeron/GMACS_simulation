...
GenData <- function(Results,Determ=F)
{
 ...
 # Length data (Stage 1 - select true values)
 ...
 # Length data (Stage 2 - generation)
...
GenTrue <- function(Isim,Orig,Nrec,SigmaR,IsRecRecRand=T)
{
  file.copy(from=paste0(Orig$OrigFolder,"gmacs.exe"),to=paste0(TrueFolder,"gmacs.exe"))
  
  Index <- FindLoc(Orig$ParFile,"rec_dev_est:",2)
  Par2 <- Orig$ParFile
  if (IsRecRecRand==F) {for (Iyr in 1:Nrec) Par2[Index+1,Iyr] <- as.numeric(Orig$ParFile[Index+1,Iyr])+rnorm(1,0,SigmaR)}
  if (IsRecRecRand==T) {for (Iyr in 1:Nrec) Par2[Index+1,Iyr] <- rnorm(1,0,SigmaR)}
  FileName <- paste(TrueFolder,"gmacs.pin",sep="")
  ...
  setwd(TrueFolder)
  TheCall <- paste("gmacs.exe -est",sep="")
  AA <- system(TheCall,intern=T)
  file.copy(from=paste0(TrueFolder,"gmacsall.out"),to=paste0(RunFolder,"gmacsAllT",Isim,".out"),overwrite=T)
  print(paste0(TrueFolder,"gmacsall.out"))
  print(paste0(RunFolder,"gmacsAllT",Isim,".out"))
...
for (Isim in 1:Nsim)
 {  
  ...
  Orig2Sim <- GenTrue(Isim,Orig,Nrec=46,SigmaR=0.6,IsRecRecRand=T)
